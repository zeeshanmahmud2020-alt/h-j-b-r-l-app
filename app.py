...
# 1. THE LOOK: Hard-Locked Grid to stop the "puke layout"
...
if 'placed_this_turn' not in st.session_state: st.session_state.placed_this_turn = [] # Track (r, c, char)

# 3. HEADER
st.markdown(f"<h2 style='text-align:center;'>Score - P1: {st.session_state.scores['P1']} | P2: {st.session_state.scores['P2']}</h2>", unsafe_allow_html=True)
    ...
    for c in range(11):
        ...
        if cols[c].button(val if val else " ", key=f"b_{r}_{c}"):
            if st.session_state.sel_idx is not None:
                ...
                st.session_state.placed_this_turn.append((r, c, char))
...
# 6. COMPLEX RULES: Submit & Self-Heal
if st.button("🔥 SUBMIT WORD", use_container_width=True, type="primary"):
    # Read the word from the board (horizontal/vertical scan logic)
    word = "".join([d[2] for d in st.session_state.placed_this_turn])
    
    if word in WORDS_DB:
        st.session_state.scores[st.session_state.turn] += len(word)
        st.session_state.turn = "P2" if st.session_state.turn == "P1" else "P1"
        st.session_state.placed_this_turn = []
        st.success(f"'{word}' Accepted!")
        st.rerun()
    else:
        # GIBBERISH REJECTION: Clean the board of bad letters
        for r, c, char in st.session_state.placed_this_turn:
            st.session_state.board[r][c] = ""
        st.session_state.placed_this_turn = []
        st.error("Gibberish detected! Tiles returned.")
        st.rerun()
